...
export default function CreateInvoicePage() {
  ...
  const createInvoiceMutation = useMutation({
    mutationFn: async (args: { send: boolean }) => {
      ...
      const created = await apiClient.createInvoice({
      ...
      if (args.send) {
        await apiClient.sendInvoice(created.id);
      }
    ...
    onSuccess: async (invoice, variables) => {
      ...
      toast.success(variables.send ? "Invoice sent" : "Invoice saved");
  ...
  const handleSaveDraft = () => {
    setIsLoading(true);
    createInvoiceMutation.mutate({ send: false });
  };

  const handleSendInvoice = () => {
    setIsLoading(true);
    createInvoiceMutation.mutate({ send: true });
  };
            ...
            >
              <Card className="premium-card">
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button variant="outline" onClick={addLineItem} className="w-full mt-4" disabled={isLoading}>
                    <Plus className="w-4 h-4 mr-2" />
                    Add Line Item
                  </Button>
                </CardContent>
